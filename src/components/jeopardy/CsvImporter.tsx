...
import { parseQuizCsv } from '@/games/jeopardy/csv';
...
export function CsvImporter({ onImported }: CsvImporterProps) {
  ...
  const handleFile = async (file: File) => {
    ...
    try {
      ...
      const result = parseQuizCsv(text, { name: name || defaultName(file.name) });
      if (!result.ok) {
  ...
  const handleSave = () => {
    ...
    const result = parseQuizCsv(pendingCsv, { name: finalName });
    if (!result.ok) {
      setErrors(result.errors);
      toast.show({
        ...
        <div className="flex flex-col items-center text-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-fg-muted mt-1">
              Columns: <span className="font-mono">category,value,question,answer</span> · 30 rows · 6 categories × 100/200/300/400/500
            </p>
          </div>
        ...
        <div className="flex flex-col gap-3 rounded-lg border border-border bg-surface p-4">
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <Input
              ...
              placeholder="e.g. Pub Night Vol 3"
            />
          </div>
