...
import { LMS_MAX_PLAYERS, LMS_MIN_PLAYERS } from '@/games/lms/constants';
...
export function PlayerSetup({ players, onChange }: PlayerSetupProps) {
  const addPlayer = () => {
    if (players.length >= LMS_MAX_PLAYERS) return;
  ...
  const removePlayer = (id: string) => {
    if (players.length <= LMS_MIN_PLAYERS) return;
    ...
    <div className="flex flex-col gap-3">
        ...
        {players.map((p, i) => (
          ...
          >
            ...
            <IconButton
              ...
              disabled={players.length <= LMS_MIN_PLAYERS}
      ...
      <div className="flex items-center justify-between">
        <Button
          ...
          size="sm"
          onClick={addPlayer}
          disabled={players.length >= LMS_MAX_PLAYERS}
        >
          + Add player
        </Button>
        <span className="text-xs font-mono text-fg-muted">
          {players.length} / {LMS_MAX_PLAYERS}
        </span>
      </div>
    </div>
  );
